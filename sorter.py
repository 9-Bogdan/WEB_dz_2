...
def normalize(name):
    ...
    finally_name = "".join(
        ch if ch.isalnum() else "_" for ch in normalize_name)
...
def sort_file():
    ...
    while True:
        folder_name = input(
            'Введіть шлях до папки, в якій потрібно відсортувати файли або exit для виходу: ')
        if folder_name == "exit":
            break
            ...
            if path.is_dir():
                ...
                for item in items:
                    ...
                    try:
                        ...
                        print(
                            f"Файл {item.name} зайнятий програмою або процесом")

            else:
                print(f"{path} це файл")

            count_files(count_list)
            # si.result(count_list)
