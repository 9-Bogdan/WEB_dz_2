...
class NotesInterface(Interface):
    def commands(self, command):
        return command

    def notes(self, note_book):
        results = ''
        for record in note_book.data.values():
            results += "\n"+f"Назва нотатки: '{record.name.value}'" + \
                "".join(f"\n#{tag.value}" for tag in record.tags)
            if record.text_note:
                results += ' \nТекст нотатки: ' + str(record.text_note.value)
        if not results:
...
def main_note_book():
    ...
    while True:
        print('\nДоступні команди:')
        print(f"{ni.commands(command_text)}")
        ...
        if commandNote == '1':
            ...
            for name, record in sorted_records:
                print(f"{name}':"+"".join(f" {tag.value}" for tag in record.tags) +
                      ' Текст нотатки: ' + str(record.text_note.value))
...
def note_find(params=''):
    ...
    if not params:
        wordfind = input(
            "Введіть слово для пошуку (щоб шукати по тегам додайте # перед словом): ")
    else:
        wordfind = params
    if not wordfind:
...
class NoteBook(UserDict):
    ...
    def sort_records_by_tags(self):
        sorted_records = sorted(
            self.data.items(), key=lambda item: item[1].tags)
    ...
    def edite_record(self, text_name):
        ...
        if commandNote == '1':
            ...
            while True:
                ...
                print(text_mesage)
                FindTag = input(
                    'Введіть назву тега який потрібно видалити, додати чи змінити (або 0 щоб вийти з редагування): ')
                if FindTag == '0':
                    return
                find_tag = False
                for tag in record.tags:
                ...
                if not find_tag:
                    ...
                    continue
                NewTag = input(
                    'Введіть нову назву тега щоб змінити або нічого щоб видалити: ')
                if NewTag == '':
            ...
            add_text_note = input(
                'Введіть текст який потрібно додати до тексту нотатки: ')
    ...
    def __next__(self):
        ...
        chunk = list(self.data.values())[
            self._iter_index:self._iter_index + self._iter_chunk_size]
...
class CustomerBook(UserDict):
    ...
    def show_all_records(self):
        ...
        for record in self.data.values():
            results += "\n"+f"{record.name.value}:" + \
                "".join(f" {phone.value}" for phone in record.phones)
            if record.birthday:
    ...
    def days_to_birthday(self):
        if self.birthday:
            ...
            next_birthday = datetime(
                year=today.year, month=birthday.month, day=birthday.day).date()
            if today > next_birthday:
                next_birthday = datetime(
                    year=today.year+1, month=birthday.month, day=birthday.day).date()
    ...
    def _validate_birthday(value):
        ...
        if not len(value) == 10:
            raise CustomException(
                "Date of birth must have 10 characters ""1988-08-11""")
        try:
...
#     }
    ...
    #         customer_book.save_to_file(os.path.join(
    #             os.getcwd(), "customer_book.json"))
    #         note_book.save_to_file(os.path.join(os.getcwd(), "note_book.json"))
    #         print("Good bye!")
    #         break
